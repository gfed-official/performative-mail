...
- `build` binds the build-mode bar and reads category, selected building, and ghost reason text.
...
- `worldstage` runs `--host --debug-world --report= --world-dump=` and asserts live WorldStage Label3D text for Post Office, Mail, and the debug addresses, plus ConstructStage labels for the seeded wall/chest/belts/sorter and SmokeReport `constructCounts`.
- `packed-belts` runs `--host --packed-belts --report= --frame-dump=`. It asserts Playing, SmokeReport `constructCounts.belts` is `400`, and the FRAME_DUMP sidecar has `instances` at least 400, `pass=true`, and `avgMs` at most 16.7.
...
- `live-build` runs `--host --debug-world --debug-helper=build` with `--build-dump=`. It enters build mode, places `wall_wood` into `ConstructRegistry`, and asserts the live dump has `constructs=1`.
