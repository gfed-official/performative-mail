...
Host F3 lists one spawn row per content item, per mail kind, and Bike. Rows come from `DebugSpawnCatalog.From(ContentBundle)`, not a hand-maintained id list. Clicking a row on a Playing host calls `PlaySessionMachine.TrySpawn`. Guests see the rows disabled and `TrySpawn` returns false.
