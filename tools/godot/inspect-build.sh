...
grep -q 'BUILD_DUMP case=slope' "$DUMP" || fail "missing slope dump"
expect "reason=Too steep"
grep -q 'BUILD_DUMP case=missing' "$DUMP" || fail "missing missing-input dump"
expect "reason=Missing input"
