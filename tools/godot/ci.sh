...
host_interact_smoke() {
  ...
  log="$(mktemp)"
  if ! godot --headless --display-driver headless --path "$PROJECT_PATH" -- \
    --host --debug-world --debug-helper=interact --quit-after-ms=8000 --report="$report" \
    >"$log" 2>&1; then
    cat "$log"
    fail "host interact process exited non-zero"
  fi
  if [[ ! -f "$report" ]]; then
    cat "$log"
    fail "host interact did not write a report"
  fi
  cat "$report"
  echo
  grep -q '"state":"Playing"' "$report" || fail "host interact report is not Playing: $(cat "$report")"
  grep -q '"worldHash":"0x4CF184F2FA4D4EEE"' "$report" \
    || fail "host interact report missing debug worldHash: $(cat "$report")"
  need_jq
  jq -e '
    .state == "Playing"
    and .phase == "Prep"
    ...
    and .quota > 0
    and (.pawns | length) >= 1
    and (.pawns[] | select(.role == "Local") | .x == 200 and .y == 1600)
    and .worldEntityCounts.postOffices == 1
    and .worldEntityCounts.intakes == 1
    and .worldEntityCounts.houses == 2
    and .worldEntityCounts.mailboxes == 2
    ...
    and .debugOpen == false
  ' "$report" >/dev/null \
    || fail "host interact report failed pickup-deliver: $(cat "$report")"
}
...
host_live_overlay_smoke() {
  ...
  if [[ ! -f "$dump" ]]; then
    cat "$log"
    fail "host live-overlay did not write an overlay dump"
  fi
  cat "$report"
  echo
  echo "---- $dump ----"
  cat "$dump"
  echo
  grep -q '"state":"Playing"' "$report" || fail "host live-overlay report is not Playing: $(cat "$report")"
  grep -q '"worldHash":"0x4CF184F2FA4D4EEE"' "$report" \
    || fail "host live-overlay report missing debug worldHash: $(cat "$report")"
  need_jq
  jq -e '
    .state == "Playing"
    and .phase == "Prep"
    ...
    and .worldEntityCounts.houses == 2
    and .worldEntityCounts.mailboxes == 2
    and .overlayOpen == true
    and .debugOpen == false
  ' "$report" >/dev/null \
    || fail "host live-overlay report failed jq schema: $(cat "$report")"
  grep -q 'OVERLAY_DUMP case=live' "$dump" || fail "missing live overlay dump: $(cat "$dump")"
  grep -q 'OVERLAY_DUMP_END' "$dump" || fail "missing OVERLAY_DUMP_END: $(cat "$dump")"
  ...
  if grep -Fqx "hotbar_1_0 text=1 13 opacity=0.6" "$dump"; then
    fail "live overlay dump still has placeholder pending cell: $(cat "$dump")"
  fi
}
...
host_live_hud_smoke() {
  ...
  grep -Fqx "CompassFacing=N" "$dump" || fail "live HUD dump missing compass facing: $(cat "$dump")"
  grep -Fqx "CompassDistrict=1" "$dump" || fail "live HUD dump missing nearby district: $(cat "$dump")"
  grep -Fqx "CompassSlots=000000000011111110000000" "$dump" \
    || fail "live HUD dump missing district-1 compass band: $(cat "$dump")"
